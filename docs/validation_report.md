...
## Fresh recognition audit — 2026-09-05

Baseline `ce06494` was independently checked against PostgreSQL, source MP4 pixels, current model execution and machine consensus. Delivered code `13759e8` adds separate human labels/source timeline and a versioned numeric OCR cadence fix. **48 Pytest tests (22 unit, 26 PostgreSQL), 1 Vitest, 4 actual Chromium checks and the complete native `make validate` passed.** Generated-client consistency, clean migration through `20260907_labels`, synthetic process E2E and actual video seek/decode were exercised. The new model run recovered from intentional SIGKILL and duplicate replay without new business effects: 25 passages, 2 accepted/8 review/15 rejected, 42 exact crop checks, 102 verified evidence objects. Original run/evidence and blank human labels were preserved.

See [progress audit](progress_audit.md) for exact commands, identifiers, resolved failures, the requirement matrix and unexecuted gates. Docker remains absent, NVIDIA driver unusable, hosted CI unexecuted, human recognition/count accuracy unmeasured. Historical sections above are milestone reports, not substitutes for these fresh checks.
