...
Requires both local demo authentication and `ROADEYE_RECORDED_ENABLED=true`. `/v1/recordings` exposes the fixed registry without accepting paths. `POST /v1/recorded/runs` accepts recording ID, duration (default 60 seconds), assigned UTC anchor, ROI top and crossing line. It requires administrator authorization and an idempotency key, returning 202 after durable receipt. Same-key/different-body requests conflict.

Investigators/admins can list `/v1/recorded/runs`, poll `/v1/recorded/runs/{run_id}`, and inspect `/v1/recorded/runs/{run_id}/passages`. Status derives received/processing/completed/failed from the actual outbox and includes frame progress, attempts, errors, independent vehicle counts and consensus outcomes. `POST /v1/recorded/runs/{run_id}/replay` is an administrator action and refuses an active worker lease. Evidence retrieval uses the existing authorized `/v1/evidence/{id}` endpoint with JPEG/PNG MIME and digest validation. The public input route remains synthetic-only; model-origin inputs can only enter from the server worker. Generic synthetic analytics/trajectory/control endpoints explicitly reject recorded runs.
