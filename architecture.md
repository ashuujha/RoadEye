...
`evaluation_labels` stores append-only run/target revisions with authenticated actor, self-reported reviewer name, source-relative time and independent human transcription/readability/vehicle/detection assessments. Targets are predicted passages, separately marked missed vehicles, or whole-timeline coverage declarations. Labels never enter the inference worker. Evaluation compares immutable machine decisions, not operationally corrected observations. Complete-run metrics remain null until required labels and timeline coverage exist.

The authenticated recording-registry endpoint serves the original MP4 with byte-range support for local timeline inspection; it verifies the registered source digest and audits access. It does not accept browser paths. Evidence SVG overlays use original pixels/boxes; old crop samples with no same-frame vehicle box are explicitly marked. New `recorded-onnx-v2` samples retain that box and use a numerical tolerance at the 600 ms OCR scheduling boundary. Original v1 sampling semantics and raw records remain intact; model/consensus thresholds are unchanged. New configurations snapshot processing/inference file digests. Independent labels, raw OCR and operational corrections are distinct data products.

No appearance-assisted cross-camera subsystem exists. The [RoundaboutHD plan](docs/dataset_decision.md) describes a future isolated tracklet experiment, not current functionality. Fixed camera/lane contracts and synthetic graph geometry must be generalized before real multi-camera enrollment.
